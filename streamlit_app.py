...
with st.sidebar:
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        with st.expander("Project Specification & Progress Dates", expanded=False):
            ...
            d1 = st.date_input("Parts Arrival", value=None, key="nd1")
            d2 = st.date_input("Installation Complete", value=None, key="nd2")
            d3 = st.date_input("Testing Complete", value=None, key="nd3")
            d4 = st.date_input("Cleaning Complete", value=None, key="nd4")
            d5 = st.date_input("Delivery Complete", value=None, key="nd5")
                ...
                """, unsafe_allow_html=True)
                    ...
                    if st.session_state.get("editing_index") == idx:
                        ...
                        with st.form(key=f"edit_form_{idx}"):
                            c1, c2 = st.columns(2)
                            with c1:
                                e_type = st.selectbox("Project Type*",
                                                      ["Enclosure", "Open Set", "Scania", "Marine", "K50G3"],
                                                      index=["Enclosure", "Open Set", "Scania", "Marine",
                                                             "K50G3"].index(row["Project_Type"]) if row[
                                                                                                        "Project_Type"] in [
                                                                                                        "Enclosure",
                                                                                                        "Open Set",
                                                                                                        "Scania",
                                ...
                                e_year = st.selectbox("Year*", [2024, 2025, 2026],
                                                      index=[2024, 2025, 2026].index(row["Year"]) if row["Year"] in [
                                                          2024, 2025, 2026] else 1)
                            ...
                            with st.expander("Project Specification & Progress Dates", expanded=True):
                                ...
                                e_d2 = st.date_input("Installation Complete", value=pd.to_datetime(
                                    row["Installation_Complete"]).date() if pd.notna(
                            ...
                            with col_save:
                                if st.form_submit_button("Save Changes", type="primary"):
                                    if not e_name.strip():
                                        ...
                                        new_spec = f"Genset model: {e_s1 or —}\nAlternator Model: {e_s2 or —}\nController: {e_s3 or —}\nCircuit breaker Size: {e_s4 or —}\nCharger: {e_s5 or —}"
                                        ...
                                        df.at[idx, "Installation_Complete"] = e_d2.strftime(
                                            "%Y-%m-%d") if e_d2 else None
    ...
    st.markdown("---")
    st.caption("All projects permanently saved • Edit & Delete work perfectly • Progress auto-updates")
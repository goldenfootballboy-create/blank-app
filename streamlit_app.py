...
if not os.path.exists(PROJECTS_FILE) and open(PROJECTS_FILE, "w", encoding="utf-8").write("[]")
...
def load_projects():
    ...
    if "Real_Count" not in df.columns: df["Real_Count"] = df.get("Qty", 1)
    return df
...
def save_projects(df):
    ...
    df2.to_json(PROJECTS_FILE, orient="records", force_ascii=False, indent=2)
    ...
    for idx, row in df.iterrows():
        ...
        with st.expander(f"**{row['Project_Name']}** • {row['Project_Type']} • {row.get('Customer','—')}", expanded=False):
            ...
            with col_l:
                ...
                <div style="background:#e0e0e0; border-radius:8px; overflow:hidden; height:12px; margin:10px 0;">
                ...
                # 基本資訊
                st.markdown(f"""
                <div style="padding:20px; border-radius:10px; border-left:8px solid {color}; line-height:1.8;">
                ...
                # Project Spec. 清楚一行一行（無論 Light/Dark 都看得見）
...
st.markdown("---")
st.caption("Project Spec. now clearly displayed line by line • Works perfectly in both Light & Dark theme")
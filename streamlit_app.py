...
if len(df) == 0:
    ...
    for idx, row in df.iterrows():
        ...
        # Progress Reminder（安全讀取 + 完全不殘留）
        reminder_text = str(row.get("Progress_Reminder", "")).strip()
        reminder_display = f'<div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); font-weight:bold; font-size:1.1rem; color:white; text-shadow:1px 1px black; pointer-events:none; z-index:10;">{reminder_text}</div>' if reminder_text else ""
...
st.markdown("---")
st.caption("Progress Reminder perfect • No HTML residue • All functions work • 永久儲存")
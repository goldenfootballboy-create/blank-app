...
with st.sidebar:
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        with col1:
            new_type = st.selectbox("Project Type*", ["Required"]
            ", ["
            Enclosure
            ","
            Open
            ...
            new_leadtime = st.date_input("Lead Time ["
            Required
            "]", value = date.today())
            ...
            st.markdown("**Progress Dates** (optional)")
            p1 = st.date_input("Parts Arrival", value=None, key="np1")
            p2 = st.date_input("Installation Complete", value=None, key="np2")
            p3 = st.date_input("Testing Complete", value=None, key="np3")
            p4 = st.date_input("Cleaning Complete", value=None, key="np4")
            p5 = st.date_input("Delivery Complete", value=None, key="np5")
            ...
            st.markdown(f"""
                <div style="background:#f8f9fa; padding:20px; border-radius:10px; border-left:8px solid {color};">
                    <p style="margin:5px 0;"><strong>Year:</strong> {row['Year']} | <strong>Lead Time:</strong> {row['Lead_Time']}</p>
                    <p style="margin:5px 0;"><strong>Customer:</strong> {row.get('Customer', '—')} | <strong>Supervisor:</strong> {row.get('Supervisor', '—')} | <strong>Qty:</strong> {row.get('Qty', 0)}</p>
                    ...
                    {f"<p style='margin:15px 0; line-height:1.6;'><strong>Description:</strong><br>{row.get('Description', '—')}</p>" if row.get('Description') else ""}
            ...
            with col_btn:
                if
            st.button("Edit", key=f"edit_{idx}", use_container_width=True):
            ...
            st.markdown("---")
            st.caption(
                "All projects permanently saved • Progress auto-calculated from 5 stages • Color changes with progress")
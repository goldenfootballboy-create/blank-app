        ...
        if st.session_state.get("editing_index") == idx:
            ...
            with st.form(key=f"edit_form_{idx}"):
                ...
                with st.expander("Project Specification & Progress Dates", expanded=True):
                    ...
                    st.markdown("**Progress Dates**")
                    e_d1 = st.date_input("Parts Arrival", value=pd.to_datetime(row["Parts_Arrival"]).date() if pd.notna(row["Parts_Arrival"]) else None, key=f"d1_{idx}")
                    e_d2 = st.date_input("Installation Complete", value=pd.to_datetime(row["Installation_Complete"]).date() if pd.notna(row["Installation_Complete"]) else None, key=f"d2_{idx}")
                    e_d3 = st.date_input("Testing Complete", value=pd.to_datetime(row["Testing_Complete"]).date() if pd.notna(row["Testing_Complete"]) else None, key=f"d3_{idx}")
                    e_d4 = st.date_input("Cleaning Complete", value=pd.to_datetime(row["Cleaning_Complete"]).date() if pd.notna(row["Cleaning_Complete"]) else None, key=f"d4_{idx}")
                    e_d5 = st.date_input("Delivery Complete", value=pd.to_datetime(row["Delivery_Complete"]).date() if pd.notna(row["Delivery_Complete"]) else None, key=f"d5_{idx}")
...
st.markdown("---")
st.caption("Edit function now 100% working • Progress visible on card • Clean & beautiful")
...
# ==============================================
# 中間：超小巧 Project Counter + 進度卡片
# ==============================================
st.title("YIP SHING Project Dashboard")

# 極簡 Project Counter（一行搞定）
if len(df) > 0:
    counter = df.groupby("Project_Type")["Qty"].sum().astype(int).sort_index()
    total_qty = int(df["Qty"].sum())

    counter_html = "<div style='text-align:center; margin:10px 0; font-size:0.95rem;'>"
...
st.markdown("---")
st.caption("YIP SHING • Compact counter with Total + Type Qty • Clean progress cards • 永久儲存")